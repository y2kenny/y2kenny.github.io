...
for tag in tags
    ...
    write_template_file(tagpage_path, "/archive/tags/#{tagpath}/", tag, {tag: tag})
end
# Create template files for each year and month
for date in dates
    yearpage_path = dates_folder_path + "/#{date[:year]}.md"
    write_template_file(yearpage_path, "/archive/dates/#{date[:year]}/", date[:year], {year:"#{date[:year]}"})

    monthpage_path = dates_folder_path + "/#{date[:year]}-#{date[:month]}.md"
    month_name = "#{MONTH_NAMES[date[:month].to_i]} #{date[:year]}"
    write_template_file(monthpage_path, "/archive/dates/#{date[:year]}/#{date[:month]}/", month_name, {year: date[:year], month: date[:month]})
end
